...
export const Captcha = {
    ...
    initialize: function (loaderUrl, selector = null, options = {}) {
        ...
        if (this.initialized) {
            Utils.warn('Already initialized. Cannot use Captcha.initliaze() twice.');
    ...
    loadFromSelector: function (selector, options = {}) {
        ...
        const newInstances = Array.from(document.querySelectorAll(selector))
            .map((element) => Widget.create(element, options))
    ...
    checkIfValid: function () {
        if (!this.initialized) {
            Utils.warn('Captcha has not been initialized.');
            return false;
        }

        if (this.instances.length === 0) {
            Utils.warn('No instances found.');
            return false;
        }
