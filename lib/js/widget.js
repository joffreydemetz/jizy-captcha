...
export class Widget {
    /**
     * Validates input and returns a new Widget, or null if the element/options
     * cannot produce a working widget. Prefer this over `new Widget(...)` —
     * a constructor cannot actually short-circuit with `return null`, so the
     * raw constructor would otherwise hand back a half-initialized instance.
    ...
    static create($element, options) {
        ...
        if (!options || !options.path) {
            Utils.error('The option "path" has not been set.');
            return null;
        }
    ...
    /**
     * Creates a new Widget instance. Prefer `Widget.create()` for callers that
     * may pass invalid input — this constructor assumes pre-validated args.
     * @param {HTMLElement} $element The DOM element to generate the widget into.
